  ...
  const createPaymentIntent = async () => {
    try {
      ...
      if (data.success) {
        ...
        console.log(`[Stripe Checkout] Order total: $${data.amount.toFixed(2)}`);
        console.log(`[Stripe Checkout] Items: ${data.items.length}`);
        
        // Show order breakdown
        if (data.items.length > 1) {
          const sellerCount = new Set(data.items.map(item => item.seller_id)).size;
        ...
        console.error('[Stripe Checkout] Payment intent creation failed:', data.error);
      }
    } catch (error) {
  ...
  const createOrderAfterPayment = async (paymentIntentId) => {
    try {
      ...
      if (data.success) {
        ...
        console.log(`[Order Creation] Order ID: ${data.order_id}`);
        
        const splitResult = data.split_result;
        ...
        console.error('[Order Creation] Order creation failed:', data.error);
        ...
        if (onPaymentError) {
          onPaymentError(new Error(data.error));
        }
      }
    } catch (error) {
...
const StripeCheckoutForm = ({ cartItems, shippingAddress, onPaymentSuccess, onPaymentError }) => {
  return (
